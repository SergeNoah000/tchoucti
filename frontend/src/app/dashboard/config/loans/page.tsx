...
export default function ConfigLoansPage() {
  ...
  return <ConfigLoansInner association={association} />;
}

function ConfigLoansInner({ association }: { association: Association }) {
  const t = useTranslations("configLoans");
  ...
  // Caisses projet exclues comme source — fonds finis, ne peuvent pas alimenter
  // un prêt récurrent.
  ...
  return (
    <div className="space-y-6">
      ...
      {!enabled && (
        <ConfigPreview intent="warning">{t("disabledPreview")}</ConfigPreview>
      )}
    ...
    <div className="space-y-4 rounded-lg border-2 border-primary/30 bg-primary/5 p-4">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <HelpField
          label={t("typeName")}
          required
          example={t("typeNameExample")}
        >
          <Input value={name} onChange={(e) => setName(e.target.value)} />
        </HelpField>
        <HelpField
          label={t("sourceCaisse")}
          hint={t("sourceCaisseHint")}
          required
        >
