...
export default function ActivatePage() {
  ...
  const existingActive = !!peek?.existing_active;
  const orgName = peek?.association_name || peek?.groupement_name;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await invitationsApi.accept({
        token,
        password: existingActive ? undefined : pwd,
            ...
            {done ? (
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <Button type="submit" size="lg" className="w-full gap-2" disabled={loading}>
                  ...
                  {loading ? t("activating") : existingActive ? t("join") : t("activate")}
                </Button>
              </form>
            )}
          </CardContent>
